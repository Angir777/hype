<?php
  ...
  */
  ...
  $getLogin = new Login();
  $showcPanelForm = $getLogin->getForm($l);
  // work
  $l_error = '';
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["l_name"]) OR empty($_POST["l_password"])) {
      ...
      $loginName = check_inputs($_POST["l_name"]);
      $loginPassword = check_inputs($_POST["l_password"]);
      $infoLogin = $getLogin->getLogin($loginName, $loginPassword);
      if ($infoLogin != null) {
        $l_error = '<div class="alert alert-danger">'.$l[$infoLogin].'</div>';
      } else {
